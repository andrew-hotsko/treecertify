...
// ---------------------------------------------------------------------------
// Shared arborist-like interface for PDF generation
// ---------------------------------------------------------------------------

interface ArboristLike {
  name: string;
...
async function generateSamplePdfBuffer(arboristData: ArboristLike): Promise<Buffer> {
  ...
  // Logo
  let logoBase64 = "";
  if (arboristData.companyLogoUrl) {
    const b64 = logoToBase64(arboristData.companyLogoUrl);
  ...
  try {
    additionalCertsArr = JSON.parse(arboristData.additionalCerts || "[]");
  } catch { /* skip */ }
  ...
  <div class="signature-block">
    ...
    <div class="sig-area">
      <div class="sig-line-group">
        <div class="sig-esignature">${esc(arboristData.signatureName || arboristData.name)}</div>
        <div class="sig-rule"></div>
        <div class="sig-label">Signature</div>
      </div>
      <div class="sig-details">
        <div class="sig-detail-row"><span class="sig-detail-label">Name:</span> ${esc(arboristData.name)}</div>
        <div class="sig-detail-row"><span class="sig-detail-label">ISA Certified Arborist:</span> #${esc(arboristData.isaCertificationNum)}</div>
        ${arboristData.traqCertified ? `<div class="sig-detail-row"><span class="sig-detail-label">Qualification:</span> ISA Tree Risk Assessment Qualified</div>` : ""}
        ...
        ${arboristData.licenseNumbers ? `<div class="sig-detail-row"><span class="sig-detail-label">License(s):</span> ${esc(arboristData.licenseNumbers)}</div>` : ""}
        ${arboristData.companyName ? `<div class="sig-detail-row"><span class="sig-detail-label">Company:</span> ${esc(arboristData.companyName)}</div>` : ""}
        <div class="sig-detail-row"><span class="sig-detail-label">Date:</span> ${dateStr}</div>
      </div>
    </div>
  </div>`;

  const headerCompany = esc(arboristData.companyName || arboristData.name);
...
<div class="cover-page">
  <div class="cover-company-block">
    ...
    <div class="cover-company-name">${esc(arboristData.companyName || arboristData.name)}</div>
    ${arboristData.companyAddress ? `<div class="cover-company-detail">${esc(arboristData.companyAddress)}</div>` : ""}
    <div class="cover-company-detail">
      ${[arboristData.companyPhone, arboristData.companyEmail, arboristData.companyWebsite].filter(Boolean).map((v) => esc(v!)).join(" &nbsp;&bull;&nbsp; ")}
    </div>
  </div>
  ...
  <div class="cover-footer">
    <div class="cover-credentials-line">${esc(arboristData.name)} &nbsp;&bull;&nbsp; ISA Certified Arborist #${esc(arboristData.isaCertificationNum)}</div>
    ${arboristData.traqCertified ? '<div class="cover-credentials-line">ISA Tree Risk Assessment Qualified</div>' : ""}
...
// ---------------------------------------------------------------------------
// Showcase arborist data (sample certifier — not personalized)
// ---------------------------------------------------------------------------

const SHOWCASE_ARBORIST: ArboristLike = {
  name: SAMPLE_REPORT.certifier.name,
  isaCertificationNum: SAMPLE_REPORT.certifier.isaCertNum,
  traqCertified: SAMPLE_REPORT.certifier.traqQualified,
  companyName: SAMPLE_REPORT.certifier.company,
  companyAddress: SAMPLE_REPORT.certifier.companyAddress,
  companyPhone: SAMPLE_REPORT.certifier.companyPhone,
  companyEmail: SAMPLE_REPORT.certifier.companyEmail,
  companyWebsite: null,
  companyLogoUrl: null,
  signatureName: null,
  licenseNumbers: null,
...
//   ?showcase=true → returns a universal showcase PDF (Jane Rodriguez)
//   (no param)     → returns cached per-arborist sample PDF
// ---------------------------------------------------------------------------
...
export async function GET(request: NextRequest) {
  ...
  if (showcase) {
    ...
    const cachePath = path.join(cacheDir, "showcase.pdf");
    ...
    // Generate and cache showcase PDF
    try {
      const pdfBuffer = await generateSamplePdfBuffer(SHOWCASE_ARBORIST);
      ...
      console.error("Showcase PDF generation error:", error);
      return NextResponse.json(
        { error: "Failed to generate showcase PDF" },
        { status: 500 }
      );
    }
  }

  // Non-showcase: return per-arborist cached sample
  try {
...
export async function POST() {
  try {
    ...
    // Generate the PDF with arborist's own branding
    const pdfBuffer = await generateSamplePdfBuffer(arborist as ArboristLike);
