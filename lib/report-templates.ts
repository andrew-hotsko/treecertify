...
export const REPORT_TEMPLATES: ReportTemplate[] = [
  ...
  },
  {
    ...
    aiPromptAdditions: `Use the CTLA Trunk Formula Method for valuation. When type-specific valuation data is provided (trunk area, species rating %, condition rating %, location rating %, cost per sq inch, appraised value), use these pre-calculated values directly — do not recalculate them. Present the per-tree valuation breakdown showing all factors. Include an aggregate total value for all trees. Use realistic species ratings based on the Western Chapter ISA species classification guide. Format currency values clearly. Present the valuation calculations clearly so a property owner or insurance adjuster can follow the math. Show the formula: Appraised Value = Trunk Area × Cost per Sq Inch × Species Rating × Condition Rating × Location Rating.`,
  },
  {
